			...
			<form class="lockscreen animated flipInY" action="javascript:void(0);">
				...
				<div class="row">
						...
						<h1>${productName!""}&nbsp;
							...
							<i id="checkIcon" class="fa fa-lock text-danger"> 锁定</i>
							
						</h1>
						<div class='font-xs margin-top-5 text-muted'><code>${lockMessage!''}</code></div>
						<div id="applyResult">
					...
					$("#activate").click(function(){
						$.post("${springMacroRequestContext.contextPath}/activate",{},function(data){
							...
							if(data.success == "valid_license"){
								...
								var fun = function(){
									if (i == 0) { 
										window.location = "${springMacroRequestContext.contextPath}/login"; 
